...
def compute_A_regularized_nnls(X: np.ndarray, 
                               Z: np.ndarray, 
                               A: np.ndarray = None,
    ...
    A = np.array(
        [scipy.optimize.nnls(A=Zt_padded, b=X_padded[n, :])[0] for n in range(X.shape[0])]
        )
    return A


def compute_B_regularized_nnls(X: np.ndarray, 
                               A: np.ndarray, 
                               B: np.ndarray = None,
                               derivative_max_iter = None,
                               ) -> np.ndarray:
    ...
    B = np.array(
        [scipy.optimize.nnls(A=Xt_padded, b=Z_padded[k, :])[0] for k in range(Z.shape[0])]
        )
    return B


@njit(cache=True)
def compute_A_projected_gradients(X: np.ndarray, 
                                  Z: np.ndarray, 
                                  A: np.ndarray,
    ...
    for _ in range(derivative_max_iter):
        ...
        prev_A = A
        for _ in range(derivative_max_iter*100):  # Base implementation has a while True
...
@njit(cache=True)
def compute_B_projected_gradients(X: np.ndarray, 
                                  A: np.ndarray, 
                                  B: np.ndarray,
                                  derivative_max_iter: int,
                                  ) -> np.ndarray:
...
@njit(cache=True)
def compute_A_frank_wolfe(X: np.ndarray, 
                          Z: np.ndarray, 
                          A: np.ndarray = None,
...
@njit(cache=True)
def compute_B_frank_wolfe(X: np.ndarray, 
                          A: np.ndarray,
                          B = None,
