...
def hvg_vst_method(adata):
    ...
    df["variance"] = adata_subset.X.todense().var(axis=0).A1 if issparse(adata_subset.X) else adata_subset.X.var(axis=0)
    ...
    tmp_mtx = adata_subset.X.copy().T.todense().astype(np.float64) if issparse(adata_subset.X) else adata_subset.X.copy().T.astype(np.float64)
    ...
    adata.var["std_variance_rank"] = (adata.var["std_variance"]*-1).argsort().argsort()
...
def get_var_explained_pca(X, random_state=42, max_comp=np.inf):
    ...
    assert np.isclose(total_variance, np.sum((X.shape[0]-1)/X.shape[0] * pca.explained_variance_))
    ...
    plot_df = pd.DataFrame({"component": np.arange(1, len(pca.explained_variance_)+1),
                        "Variance Explained": pca.explained_variance_,
                        "Fraction of Variance Explained": (pca.explained_variance_ / total_variance),
                        "Cumulative Fraction of Variance Explained": np.cumsum(pca.explained_variance_) / total_variance
                        })
    ...
    plot_df["var_type"] = pd.Categorical(plot_df["var_type"], categories=["Fraction of Variance Explained", "Cumulative Fraction of Variance Explained"])
...
def get_var_explained_aa(X, k_arr=np.arange(2,10)):
    ...
    plot_df = pd.DataFrame({"k": k_arr,
                            "varexpl": np.array([results[k]["varexpl"] for k in k_arr]),
                            "varexpl_ontop": np.concatenate((varexpl[0][None], varexpl[1:] - varexpl[:-1]))})
    ...
    proj_val = (proj_mtx @ (plot_df.values[:, :2] - plot_df.values[:, :2][0, ]).T).T + offset_vec
    ...
    p2 = pn.ggplot() + \
        ...
        pn.geom_segment(data=dist_df, mapping=pn.aes(x="k", y=0, xend="k", yend="dist"), color="blue") + \
        pn.scale_x_continuous(breaks=np.arange(k_arr.min(), k_arr.max()+1)) + \
        pn.labs(x="Component (Number of Archetypes)", y="Distance to Projected Point")
    
    p3 = pn.ggplot(plot_df) + \
        pn.geom_point(pn.aes(x="k", y="varexpl_ontop"), color="grey") + \
        pn.geom_line(pn.aes(x="k", y="varexpl_ontop"), color="grey") + \
        pn.labs(x="k (Number of Archetypes)", y="Variance Explained on Top of (k-1) Model") + \
        pn.scale_x_continuous(breaks=np.arange(k_arr.min(), k_arr.max()+1)) + \
        pn.lims(y=(0, None))
...
def bootstrap_variance_single_k(X, n_bootstrap, k, seed=42, **kwargs):
    ...
    Z_stack = np.stack([align_archetypes(ref_arch=ref_Z, query_arch=query_Z) for query_Z in Z_stack])
...
def bootstrap_variance_k_arr(X, n_bootstrap, k_arr, delta=0, seed=42, **kwargs):
    assert k_arr.min() > 1
    bootstrap_var = np.array([bootstrap_variance_single_k(X, n_bootstrap=n_bootstrap, k=k, delta=delta, seed=seed, **kwargs) for k in k_arr])
    plot_df = pd.DataFrame({"k": k_arr, "var": bootstrap_var})
    p = pn.ggplot(plot_df, pn.aes(x="k", y="var")) + pn.geom_point(color="blue") + pn.geom_line(color="blue") + \
...
def compute_dist_mtx(mtx_1, mtx_2):
    ...
    dist_mtx[np.isclose(dist_mtx, 0)] = 0 # avoid problems if we get small negative numbers due to numerical inaccuracies
...
def plot_2D(X, Z, color_vec, opacity=0.4):
    ...
    if color_vec is not None:
        ...
        fig = px.scatter(plot_df, x="x0", y="x1", color="color", size="marker_size", size_max=10)
    else:
    ...
    fig.add_trace(go.Scatter(x=Z_plot[0, ], y=Z_plot[1, ], fill="toself", opacity=opacity))
...
def plot_3D(X, Z, color_vec=None, opacity=0.2):
    ...
    if color_vec is not None:
        plot_df["color"] = color_vec
        fig = px.scatter_3d(plot_df, x="x0", y="x1", z="x2", color="color", size="marker_size", size_max=10)
    else:
        fig = px.scatter_3d(plot_df, x="x0", y="x1", z="x2", size="marker_size", size_max=10)
    ...
    Z_plot = Z_plot[:, order]
    Z_plot = np.column_stack((Z_plot, Z_plot[:, 0]))
    fig.add_trace(go.Scatter3d(x=Z_plot[0,:], y=Z_plot[1,:], z=Z_plot[2,:],
...
def plot_3D_bootstrap(bootstrap_df, color_var="archetype", opacity=0.5, iter_filter=None, jitter=0):
    import plotly.express as px
    bootstrap_df["marker_size"] = np.repeat(4, bootstrap_df.shape[0])
    if iter_filter is not None:
        bootstrap_df = bootstrap_df.loc[np.isin(bootstrap_df["iter"], iter_filter), :].copy()
    if jitter > 0:
        bootstrap_df[["pc_"+str(i) for i in range(3)]] += np.random.normal(loc=0, scale=jitter, size=bootstrap_df[["pc_"+str(i) for i in range(3)]].shape)
    fig = px.scatter_3d(bootstrap_df, x="pc_0", y="pc_1", z="pc_2", color=color_var, size="marker_size", size_max=10, opacity=opacity,
...
def plot_2D_bootstrap(bootstrap_df, color_var="archetype", opacity=0.5, iter_filter=None, jitter=0):
    import plotly.express as px
    ...
    if jitter > 0:
        bootstrap_df[["pc_"+str(i) for i in range(3)]] += np.random.normal(loc=0, scale=jitter, size=bootstrap_df[["pc_"+str(i) for i in range(3)]].shape)
    fig = px.scatter(bootstrap_df, x="pc_0", y="pc_1", color=color_var, size="marker_size", size_max=10, opacity=opacity, 
                    hover_data=["iter", "archetype", "reference"])
    fig.update_layout(template="plotly_dark")
    fig.update_yaxes(
        scaleanchor="x",
